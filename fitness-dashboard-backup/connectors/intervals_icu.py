...
class IntervalsICUConnector:
    ...
    def __init__(self, api_key: str, athlete_id: str):
        ...
        Args:
            api_key: API key from intervals.icu/settings
            athlete_id: Your athlete ID (from URL)
        """
    ...
    def get_wellness_data(self,
        ...
        """
        Get wellness data (sleep, HRV, weight, etc.)
        ...
        for entry in wellness_data:
            entry_date = entry.get("id", "")  # Date is stored as ID in format YYYY-MM-DD
    ...
    def get_fitness_trend(self,
                          ...
                          end_date: Optional[str] = None) -> List[Dict]:
        """
        Get fitness/training load data (CTL, ATL, TSB)
        ...
        Returns:
            List of fitness trend data points
        """
        if not start_date:
        ...
        # Intervals.icu returns fitness data in the wellness endpoint
        wellness_data = self.get_wellness_data(start_date, end_date)
        
        # Extract CTL, ATL, TSB from wellness
        fitness_data = []
        for entry in wellness_data:
            fitness_entry = {
                "date": entry.get("id"),
    ...
    def get_power_curve(self, activity_id: str) -> Dict:
        """
        ...
        Returns:
            Power curve data
        """
        logger.info(f"Fetching power curve for activity: {activity_id}")
        return self._make_request(f"activity/{activity_id}/power-curve")
    
    def standardize_activity(self, raw_activity: Dict) -> Dict:
        """
        Convert Intervals.icu activity to standard schema
        
        Args:
            raw_activity: Raw activity from API
        ...
        return {
            ...
            "zones": {
                "heart_rate_zones": raw_activity.get("hr_zones", []),
                "power_zones": raw_activity.get("power_zones", [])
            },
            "gear": {
                "id": raw_activity.get("gear_id"),
                "name": raw_activity.get("gear")
            },
            "location": {
    ...
    def standardize_wellness(self, raw_wellness: Dict) -> Dict:
        """
        Convert Intervals.icu wellness to standard schema
        
        Args:
            raw_wellness: Raw wellness data from API
            
        Returns:
            Standardized wellness dict
        """
        return {
...
if __name__ == "__main__":
    ...
    if not api_key:
        print("Error: INTERVALS_API_KEY not found in environment")
        exit(1)
    ...
    try:
        ...
        # Get wellness data
        wellness = connector.get_wellness_data(
            start_date=(datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
        )
